...
const SECRET_LOCATOR = /(?:bearer\s+|^[a-z][a-z0-9+.-]*:\/\/[^/\s]*@|[?&#](?:token|secret|password|api[_-]?key|x-amz-signature|signature|sig)=)/i;
...
const canonicalArtifactSource = (value) => {
  ...
  assertNoForbiddenDomainFieldsV1({artifactId: artifact.artifactId, locator: artifact.locator, mediaType: artifact.mediaType}, '$artifactSource.metadata');
  ...
  if (SECRET_LOCATOR.test(locator)) fail('SECRET_SHAPED_LOCATOR', '$artifactSource.locator must not persist credential/signed-query material', '$artifactSource.locator');
...
const canonicalInspection = (value) => {
  ...
  if (inspection.tool !== 'ffprobe' || inspection.status !== 'passed') fail('INSPECTION_REQUIRED', '$mediaInspection must be passed ffprobe evidence', '$mediaInspection');
  ...
  if (!Number.isInteger(format.sizeBytes) || format.sizeBytes < 1) fail('INVALID_FIELD', '$mediaInspection.format.sizeBytes must be positive integer', '$mediaInspection.format.sizeBytes');
  if (typeof format.durationSeconds !== 'number' || !Number.isFinite(format.durationSeconds) || format.durationSeconds <= 0) fail('INVALID_FIELD', '$mediaInspection.format.durationSeconds must be positive number', '$mediaInspection.format.durationSeconds');
  if (!Array.isArray(inspection.streams) || inspection.streams.length < 1) fail('INSPECTION_REQUIRED', '$mediaInspection.streams must not be empty', '$mediaInspection.streams');
  ...
  if (!Number.isInteger(video.width) || !Number.isInteger(video.height) || video.width < 1 || video.height < 1) fail('INSPECTION_REQUIRED', 'video stream requires positive width/height', '$mediaInspection.streams');
...
const canonicalError = (value) => {
  ...
  const message = text(error.message, '$error.message', {max: 2000});
  if (SECRET_TEXT.test(message)) fail('SECRET_SHAPED_ERROR', '$error.message must not persist credential-shaped text', '$error.message');
...
export const createSharedMediaEvidenceCollectorV1 = ({readArtifact, inspectArtifact, readRenderLog, isJobAuthorized, now = () => new Date().toISOString()} = {}) => {
  ...
  const authorize = async ({request, jobId, action}) => {
    await requireAuthorized(await authorizeJob(Object.freeze({requestId: request.requestId, inputManifestDigest: request.inputManifestDigest, jobId, action})), action);
  };
  ...
  return Object.freeze({
    async collectSucceeded({request, jobId} = {}) {
      ...
      const source = canonicalArtifactSource(await artifactReader(Object.freeze({requestId: request.requestId, inputManifestDigest: request.inputManifestDigest, jobId: normalizedJobId})));
      const inspectionData = canonicalInspection(await artifactInspector(Object.freeze({requestId: request.requestId, inputManifestDigest: request.inputManifestDigest, jobId: normalizedJobId, artifactId: source.artifactId, locator: source.locator, bytes: Buffer.from(source.artifactBytes)})));
      if (inspectionData.inspection.format.sizeBytes !== source.artifactBytes.byteLength) fail('EVIDENCE_MISMATCH', 'ffprobe size does not match collected artifact bytes', '$mediaInspection.format.sizeBytes');
      const renderLog = canonicalRenderLog(await renderLogReader(Object.freeze({requestId: request.requestId, inputManifestDigest: request.inputManifestDigest, jobId: normalizedJobId})));
      const artifactSha256 = sha256(source.artifactBytes);
      const collectedAt = canonicalTimestamp(await clock());
      const artifact = {artifactId: source.artifactId, locator: source.locator, mediaType: source.mediaType, byteLength: source.artifactBytes.byteLength, sha256: artifactSha256, durationSeconds: inspectionData.inspection.format.durationSeconds, width: inspectionData.video.width, height: inspectionData.video.height, container: inspectionData.container, videoCodec: inspectionData.videoCodec, ...(inspectionData.audioCodec ? {audioCodec: inspectionData.audioCodec} : {})};
      const evidence = {contractVersion: MEDIA_RENDER_V1, messageType: 'evidence', requestId: request.requestId, jobId: normalizedJobId, inputManifestDigest: request.inputManifestDigest, artifactSha256, mediaInspection: inspectionData.inspection, renderLog: {sha256: sha256(renderLog), byteLength: renderLog.byteLength}, collectedAt};
      const result = {contractVersion: MEDIA_RENDER_V1, messageType: 'result', requestId: request.requestId, jobId: normalizedJobId, status: 'succeeded', artifact, evidence};
    ...
    async collectFailed({request, jobId, error} = {}) {
      ...
      const renderLog = canonicalRenderLog(await renderLogReader(Object.freeze({requestId: request.requestId, inputManifestDigest: request.inputManifestDigest, jobId: normalizedJobId})));
      const collectedAt = canonicalTimestamp(await clock());
      const result = {contractVersion: MEDIA_RENDER_V1, messageType: 'result', requestId: request.requestId, jobId: normalizedJobId, status: 'failed', evidence: {contractVersion: MEDIA_RENDER_V1, messageType: 'evidence', requestId: request.requestId, jobId: normalizedJobId, inputManifestDigest: request.inputManifestDigest, renderLog: {sha256: sha256(renderLog), byteLength: renderLog.byteLength}, collectedAt}, error: normalizedError};
      validateMediaRenderResultV1(result, {request});
      return freezeTerminalResult(result);
    },
  });
