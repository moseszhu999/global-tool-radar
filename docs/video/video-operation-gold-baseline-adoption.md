...
The Gold Remotion presentation layer now consumes the same human-approved storyboard shot timing and narration source and renders a dedicated timed caption pixel layer at the Gold mobile subtitle floor of 52 px. The component does not own or regenerate the caption timeline: Shared Media remains authoritative for deterministic cue/SRT generation, while Remotion only projects the already-approved timing/narration into pixels. Exact-head render success is still required before claiming that these caption pixels reached the MP4 artifact.

Even after exact-head render success, mobile caption readability remains a separate human full-watch gate. Timed caption rendering must not be interpreted as Gold creative approval, publication authority, or canonical `media.render.v1` terminal evidence.
