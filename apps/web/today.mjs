...
function renderStats(){
  ...
  $('[data-stat="interval"]').textContent = `${decimal(capture.intervalHours,1)} 小时`;
  $('[data-generated]').textContent = `更新于 ${dateTime(state.brief.generatedAt)}`;
  $('[data-capture-window]').textContent = `${dateTime(capture.firstCapturedAt)} 至 ${dateTime(capture.latestCapturedAt)}，当前间隔 ${decimal(capture.intervalHours,3)} 小时。`;
  const confirmed = capture.promotionGate === 'MOMENTUM_CONFIRMED';
  $('[data-gate-title]').textContent = confirmed ? '趋势已确认' : '正在增长，尚未正式确认';
  $('[data-gate-copy]').textContent = confirmed
    ? '最近两个有效快照间隔已达到确认条件。'
    : `当前只有 ${decimal(capture.intervalHours,3)} 小时，系统要求至少 ${capture.minimumConfirmationHours} 小时。`;
}
...
function renderSignals(){
  ...
  if(!signals.length){container.innerHTML = '<div class="empty">当前筛选下没有信号。这不代表数值为 0。</div>';return;}
  container.innerHTML = signals.map((signal) => `
    <article class="signal-card">
      ...
      <div class="growth-row">
        ...
        <div><small>本次增长</small><strong class="positive">+${esc(number(signal.deltaViews))}</strong></div>
        <div><small>约每小时</small><strong>+${esc(number(Math.round(signal.viewsPerHour)))}</strong></div>
      </div>
      <p class="why">${esc(signal.whyItMatters)}</p>
      <footer><span class="category">${esc(signal.category)} · ${esc(signal.claimBoundary)}</span><a class="source-link" href="${esc(signal.sourceUrl)}" target="_blank" rel="noreferrer">打开原视频</a></footer>
    </article>`).join('');
}
