...
export const PRODUCTION_POLISH_FPS = 30;
export const PRODUCTION_POLISH_FRAMES = 576;
...
const HeaderPolish: React.FC<{frame:number; beat:Beat}> = ({frame,beat}) => {
  ...
  return (
    <div style={{position:'absolute',left:56,right:56,top:72,height:295,opacity,fontFamily}}>
      <div
        style={{
          display:'inline-flex',alignItems:'center',height:40,padding:'0 16px',borderRadius:999,
          background:'rgba(9,27,50,0.88)',border:`1px solid ${C.cyan}55`,
          color:C.cyan,fontSize:19,fontWeight:900,letterSpacing:2.2,
          transform:`translateY(${lerp(10,0,eye)}px)`,opacity:eye,
        }}
      >
      ...
      <div style={{marginTop:22,maxWidth:825,fontSize:24,fontWeight:560,lineHeight:1.45,color:C.muted,letterSpacing:0.3,transform:`translateY(${lerp(14,0,sub)}px)`,opacity:sub}}>
        {beat.subtitle}
      </div>
    </div>
  );
...
const ProductionPolishOverlayV1: React.FC = () => {
  const frame=useCurrentFrame();
  return (
    <AbsoluteFill style={{background:C.key,overflow:'hidden'}}>
      <div
...
const Root: React.FC = () => (
  <Composition
    id="ToolRadarExplainerProductionPolishOverlayV1"
    component={ProductionPolishOverlayV1}
    durationInFrames={PRODUCTION_POLISH_FRAMES}
    fps={PRODUCTION_POLISH_FPS}
    width={1080}
    height={1920}
  />
);
