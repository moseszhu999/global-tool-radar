...
# Radar Scout 3D Hero v23
# V22 proved visible root-sidewall editing is the first post-V18 lever with a
# detectable phone-scale effect, but multiplying two whole control-point widths
# still produced only a modest gain. V23 localizes the same visible-surface idea:
# exact V10 centerlines and source widths stay authoritative, while sampled mesh
# width receives a smooth root-only overlap gain that decays to exactly 1.0 by
# 30% of lock length. Mid/outer/tip geometry therefore returns to V10 before the
# peak-separation zone. Exact V18 connectivity bridge + voxel profile are frozen.
...
CROWN_COUNT = 7
ROOT_OVERLAP_GAIN = 0.34
ROOT_TAPER_ZONE_END_T = 0.30
...
def localized_width_scale(t):
    if t >= ROOT_TAPER_ZONE_END_T:
        return 1.0
    u = t / ROOT_TAPER_ZONE_END_T
    return 1.0 + ROOT_OVERLAP_GAIN * (1.0 - smoothstep01(u))


def add_root_taper_clump(name, points, widths, material, *, ci):
    centers, sampled_widths = v18.v10.sample_centerline(points, widths, subdivisions=4)
    verts = []
    faces = []
    radial_segments = 16
    ...
    for ri, (center, source_width) in enumerate(zip(centers, sampled_widths)):
        ...
        _, lateral, depth = v18.v10.frame_axes(centers, ri, (0.0, -1.0, 0.0))
        visible_scale = localized_width_scale(t)
        width = source_width * visible_scale

        root_gain = 1.0 + (root_mass - 1.0) * ((1.0 - t) ** 2.2)
        half_width = max(0.016, width * root_gain)
        half_depth = max(0.012, width * 0.50 * root_gain)
    ...
    obj['sourceAuthoringMethod'] = 'V10_MASS_CLUMP_WITH_ROOT_LOCAL_TAPER_ZONE'
    obj['v10CenterlineFrozen'] = True
    obj['v10SourceWidthProfileFrozen'] = True
    obj['v10WidthRestoredAfterT'] = ROOT_TAPER_ZONE_END_T
...
def build_root_taper_union(scene):
    ...
    for ci, (points, widths) in enumerate(v18.v10.v8.V6_PRIMARY[:CROWN_COUNT], 1):
        ...
        obj = add_root_taper_clump(f'HairMassPrimaryV23_{ci:02d}', points, widths, material, ci=ci)
        v18.apply_all_modifiers(obj)
        crown.append(obj)

    bridge = v18.add_buried_root_bridge(pearl)
    union = v18.join_objects(crown + [bridge], crown[0])
    union.name = 'HairCrownRootTaperZoneUnionV23'
    union.data.name = 'HairCrownRootTaperZoneUnionV23Mesh'
    ...
    if components != 1:
        raise RuntimeError(f'expected one connected crown after V23 remesh, got {components}')

    union['sourceAuthoringMethod'] = 'V10_CROWN_ROOT_LOCAL_TAPER_WITH_V18_VOXEL_UNION'
    union['v10CrownCenterlinesFrozen'] = CROWN_COUNT
    union['v10SourceWidthsFrozenOutsideRootTaperZone'] = True
    ...
    scene['heroVersion'] = 'v23'
    scene['preferredBaselineInput'] = 'v10'
    scene['preferredRepresentationInput'] = 'v18'
    scene['controlledVariable'] = 'HAIR_CROWN_ROOT_TAPER_ZONE_AND_VALLEY_BLEND_PROFILE'
    scene['hairSurfaceAuthoringMethod'] = 'V10_CROWN_ROOT_LOCAL_TAPER_WITH_V18_VOXEL_UNION'
    scene['v10CrownCenterlinesFrozen'] = CROWN_COUNT
    scene['v10SourceWidthsFrozenOutsideRootTaperZone'] = True
...
def main():
    ...
    union, components = build_root_taper_union(scene)
    ...
    receipt = {
        'schemaVersion': 'toolradar.blender.radar-scout-hero.receipt.v23',
        'assetName': 'Radar Scout 3D Static Hero v23',
        'preferredBaselineInput': 'v10',
        'preferredRepresentationInput': 'v18',
        'controlledVariable': 'HAIR_CROWN_ROOT_TAPER_ZONE_AND_VALLEY_BLEND_PROFILE',
        'hairSurfaceAuthoringMethod': 'V10_CROWN_ROOT_LOCAL_TAPER_WITH_V18_VOXEL_UNION',
        'v10CrownCenterlinesFrozen': CROWN_COUNT,
        'v10SourceWidthsFrozenOutsideRootTaperZone': True,
