...
# Radar Scout 3D Hero v44
# V43 proved that a top-localized Laplacian can obey the low/root freeze boundary
# after explicit pin restore, but its final-pixel leverage is negligible. V44
# changes method: derive the four actual V41 crest apexes from the final mesh and
# raise only nearby upper-crest Z coordinates toward local hemispherical/ellipsoid
# shoulder envelopes. X/Y, apex vertices, roots/valleys, topology, material
# ownership/response, V41 lighting, camera and all non-crown content remain frozen.

HERE = os.path.dirname(os.path.abspath(__file__))
V41_PATH = os.path.join(HERE, 'render-radar-scout-3d-hero-v41.py')
spec = importlib.util.spec_from_file_location('radar_scout_hero_v41_for_v44', V41_PATH)
...
spec29 = importlib.util.spec_from_file_location('radar_scout_hero_v29_for_v44', V29_PATH)
...
SHOULDER_GAIN = 0.82
MAX_Z_RAISE = 0.11
PROFILE = 'FOUR_LOCAL_HEMISPHERICAL_APEX_SHOULDER_ENVELOPES_WITH_EXACT_XY_LOWZONE_APEX_FREEZE'
...
def derive_actual_apexes(mesh):
    ...
    for gi, hint in enumerate(apex_hints()):
        ...
        for v in mesh.vertices:
            ...
            if nx * nx + ny * ny <= 1.0:
                candidates.append(v)
        if not candidates:
            raise RuntimeError(f'V44 could not derive apex for group {gi + 1}')
    ...
    if len({i for i, _ in out}) != 4:
        raise RuntimeError('V44 apex derivation did not produce four distinct vertices')
    return out


def apply_v44_shoulder_envelopes(union, scene):
    mesh = union.data
    ...
    for v in mesh.vertices:
        ...
        # Upper hemisphere profile: flat derivative at the apex and increasing
        # shoulder drop outward. This broadens the high-Z shoulder without moving
        # the peak center or changing lateral coordinates.
        ...
        gain = SHOULDER_GAIN * max(0.18, height_w) * max(0.28, radial_w)
        dz = min(MAX_Z_RAISE, (target_z - float(o.z)) * gain)
        if dz <= 1e-7:
            continue
        v.co.z = float(o.z) + dz
        changed.append((v.index, gi, dz))
    ...
    if before_vertex_count != after_vertex_count:
        raise RuntimeError('V44 changed vertex count')
    if before_polygon_count != after_polygon_count:
        raise RuntimeError('V44 changed polygon count')
    if before_topology_sha != after_topology_sha:
        raise RuntimeError('V44 changed topology/material indices')
    if before_materials != after_materials:
        raise RuntimeError('V44 changed material slots')
    if before_xy_sha != after_xy_sha:
        raise RuntimeError('V44 changed X/Y coordinates')
    if low_sha_before != low_sha_after:
        raise RuntimeError('V44 changed low/root coordinates')
    if apex_sha_before != apex_sha_after:
        raise RuntimeError('V44 changed apex coordinates')
    if before_vertex_sha == after_vertex_sha or not changed:
        raise RuntimeError('V44 shoulder envelope produced no coordinate change')
    for i, _, dz in changed:
        if original[i].z <= CREST_START_Z or dz <= 0.0:
            raise RuntimeError('V44 changed a forbidden vertex')
        if abs(mesh.vertices[i].co.x - original[i].x) > 0.0 or abs(mesh.vertices[i].co.y - original[i].y) > 0.0:
            raise RuntimeError('V44 changed lateral coordinates')

    raises = [dz for _, _, dz in changed]
    apex_records = [
        {'index': i, 'coord': [float(co.x), float(co.y), float(co.z)], 'sourceHint': list(apex_hints()[gi])}
        for gi, (i, co) in enumerate(apexes)
    ]

    scene['heroVersion'] = 'v44'
    scene['controlledVariable'] = 'HAIR_CROWN_FOUR_APEX_LOCAL_HEMISPHERICAL_SHOULDER_ENVELOPE'
    scene['preferredDevelopmentInput'] = 'v41'
    scene['v41MacroRootsAndLayoutFrozen'] = True
    scene['v29V30MacroApexCentersFrozen'] = True
...
def main():
    ...
    crest = apply_v44_shoulder_envelopes(union, scene)
    ...
    receipt = {
        'schemaVersion': 'toolradar.blender.radar-scout-hero.receipt.v44',
        'assetName': 'Radar Scout 3D Static Hero v44',
        'preferredBaselineInput': 'v10',
        'preferredDevelopmentInput': 'v41',
        'controlledVariable': 'HAIR_CROWN_FOUR_APEX_LOCAL_HEMISPHERICAL_SHOULDER_ENVELOPE',
        ...
        'v29V30MacroApexCentersFrozen': True,
